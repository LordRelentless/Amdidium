...
public class TranslucentTerrainRasterizer extends Phase {
    ...
    private final Shader shader = Shader.make()
            .addSource(TASK, ShaderLoader.parse(Identifier.of("amdidium", "terrain/translucent/task.glsl")))
            .addSource(MESH, ShaderLoader.parse(Identifier.of("amdidium", "terrain/translucent/mesh.glsl")))
            .addSource(FRAGMENT, ShaderLoader.parse(
                    Identifier.of("amdidium", "terrain/frag.frag"),
    ...
    // it hijacks the unassigned indirect command dispatch and uses that to dispatch translucent chunks.
    public void raster(int regionCount, long commandAddr) {
        ...
        int lightId = ((LightMapAccessor) MinecraftClient.getInstance()
                .gameRenderer
