    ...
    private static void packageSectionGeometry(int formatSize, NativeBuffer output, short[] outOffsets,
        ...
        if (translucentData != null) {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                if (part != null) {
                    long src = MemoryUtil.memAddress(translucentData.getVertexData().getDirectBuffer())
                            + (long) part.vertexStart() * formatSize;
                    srcs[i] = src;
                    ...
                    for (int j = 0; j < part.vertexCount(); j++) {
                        ...
                        if ((j & 3) == 3) {
                            ...
                            long packedSortingData = (((long) sortDistance) << 32)
                                    | ((((long) j >> 2) << 3) | i);
            ...
            for (int i = 0; i < sortingData.length; i++) {
                ...
                copyQuad(
                        srcs[(int) (data & 7)] + ((data >> 3) & ((1L << 29) - 1)) * 4L * formatSize,
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (solid != null) {
                ...
                if (part != null) {
                    long src = MemoryUtil.memAddress(solid.getVertexData().getDirectBuffer())
                            + (long) part.vertexStart() * formatSize;
            ...
            if (cutout != null) {
                ...
                if (part != null) {
                    long src = MemoryUtil.memAddress(cutout.getVertexData().getDirectBuffer())
                            + (long) part.vertexStart() * formatSize;
                    long dst = outPtr + offset * 4L * formatSize;
                    MemoryUtil.memCopy(src, dst, (long) part.vertexCount() * formatSize);
