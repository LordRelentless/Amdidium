...
void computeOutputColour(inout vec3 colour) {
    vec3 multiplier = gl_BaryCoordNV.x*computeMultiplier(V0)
                    + gl_BaryCoordNV.y*computeMultiplier(Vp)
                    + gl_BaryCoordNV.z*computeMultiplier(V2);
    colour *= multiplier;
}
...
void main() {
    ...
    vec2 uv = gl_BaryCoordNV.x*decodeVertexUV(V0)
            + gl_BaryCoordNV.y*decodeVertexUV(Vp)
